...
- User removed CSV import/export and added a supplier filter (2026-09-24, pushed from their own PC).
...
- **User's workflow (confirmed 2026-09-25):** they edit code on their **personal computer** and push to GitHub `main`; the office PC auto-deploys. So GitHub may be ahead of this folder — always `git fetch`/check `origin/main` first, and never assume this PC has the latest code. First run of this found a bug: the scheduled task starts in System32, so auto-deploy's bare `git` calls failed silently — fixed 2026-09-25 (`Set-Location $root` + errors logged).
